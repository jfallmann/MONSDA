...
IDXPARAMS = get_always('spit_DTU_params_INDEX') ?: ''
COUNTPARAMS = get_always('spit_DTU_params_COUNT') ?: ''
DTUPARAMS = get_always('spit_DTU_params_DTU') ?: ''
...
process salmon_idx{
    ...
    """
    $COUNTBIN index $IDXPARAMS $decoy -p ${task.cpus} -t $gen -i $DTUUIDXNAME &> index.log && ln -fs $DTUUIDXNAME spit.idx
    """
...
process run_spit{
    ...
    python3 $bin $anno $ref . $DTUCOMP $PCOMBO ${task.cpus} $dparams &> log ; ln -f Tables/* . && touch Figures/dummy && ln -f Figures/* .
    """
}
...
workflow DTU{ 
    ...
    run_spit(salmon_quant.out.counts.collect(), prepare_dtu_annotation.out.anno, annofile)
