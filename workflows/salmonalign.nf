...
process salmonalign_mapping{
    ...
    if (PAIRED == 'paired'){
        ...
        """
        $MAPBIN quant -p ${task.cpus} -i $idx $stranded $MAPPARAMS --writeMappings -o $fn -1 $r1 -2 $r2 2> $lf|tee >(samtools view -h -F 4 |gzip > $pf) >(samtools view -h -f 4 |samtools collate -u -O -|samtools fastq -n -c 6 -1 $uf1 -2 $uf2 ) 2>> $lf &>/dev/null && touch $uf1 $uf2 2>> $lf &> /dev/null
        """
    }else{
        ...
        lf = "salmonalign_"+fn+".log"
        """
        $MAPBIN quant -p ${task.cpus} -i $idx $stranded $MAPPARAMS --writeMappings -o $fn -r $read 2> $lf|tee >(samtools view -h -F 4 |gzip > $pf) >(samtools view -h -f 4 |samtools fastq -n - | pigz > $uf) 2>> $lf &> /dev/null && touch $uf
        """
    }
}
