...
if ANNOPEAK is not None:
    if not rundedup:
        ...
        rule themall:
            input:  expand("PEAKS/{file}_mapped_{type}.bedg.gz",file=samplecond(SAMPLES,config), type=['sorted_dedup','unique_dedup']),
                    expand("UCSC/{file}_peak_{type}.fw.bw",file=samplecond(SAMPLES,config), type=['sorted_dedup','unique_dedup']),
                    expand("UCSC/{file}_peak_{type}.re.bw",file=samplecond(SAMPLES,config), type=['sorted_dedup','unique_dedup']),
                    expand("UCSC/{file}_peak_{type}.fw.bedg.gz",file=samplecond(SAMPLES,config), type=['sorted_dedup','unique_dedup']),
                    expand("UCSC/{file}_peak_{type}.re.bedg.gz",file=samplecond(SAMPLES,config), type=['sorted_dedup','unique_dedup']),
                    expand("PEAKS/{file}_peak_{type}.bed.gz",file=samplecond(SAMPLES,config), type=['sorted_dedup','unique_dedup']),
                    expand("PEAKS/{file}_prepeak_{type}.bed.gz",file=samplecond(SAMPLES,config), type=['sorted_dedup','unique_dedup']),
                    expand("PEAKS/{file}_peak_seq_{type}.bed.gz",file=samplecond(SAMPLES,config), type=['sorted_dedup','unique_dedup']),
    ...
    if not stranded or stranded == 'fr':
        rule BamToBed:
            input:  "MAPPED/{file}_mapped_{type}.bam",
                    "MAPPED/{file}_mapped_{type}.bam"
            output: "PEAKS/{file}_mapped_{type}.bed.gz",
                    "PEAKS/{file}_mapped_{type}.bed.gz"
