...
rule sam2bam:
    input:  "SORTED_MAPPED/{file}_mapped_{cond}_sorted.sam.gz", "QC/{file}_mapped_{cond}_sorted_fastqc.zip" if config["QC"] == "ON" else "SORTED_MAPPED/{file}_mapped_{cond}_sorted.sam.gz"
...
rule remove_dups:
    input:  "SORTED_MAPPED/{file}_mapped_{cond}_sorted.bam", "QC/{file}_mapped_{cond}_sorted_fastqc.zip" if config["QC"] == "ON" else "SORTED_MAPPED/{file}_mapped_{cond}_sorted.bam"
...
rule remove_dups_phased:
    input:  "PHASED_MAPPED/{file}_mapped_{cond}_sorted_phased.bam",
            "QC/{file}_mapped_{cond}_sorted_phased_fastqc.zip"
    output: report("PHASED_MAPPED/{file}_mapped_{cond}_sorted_phased_dedup.bam", category="PHASED"),
            "LOGS/{file}_{cond}_phased_dedup.txt",
            "PHASED_MAPPED/{file}_mapped_{cond}_sorted_phased_dedup.bam.bai"
    log:    "LOGS/{file}/remove_dups_phased_{cond}.log"
    conda:  "../envs/picardtools.yaml"
    threads: 1
    shell:  "picard MarkDuplicates I={input[0]} O={output[0]} M={output[1]} ASO=coordinate REMOVE_SEQUENCING_DUPLICATES=true REMOVE_DUPLICATES=true && samtools index {output[0]} 2> {log}"

rule fastqc_dedup:
    input:  "SORTED_MAPPED/{file}_mapped_{cond}_sorted_dedup.bam"
    output: report("QC/{file}_mapped_{cond}_sorted_dedup_fastqc.zip", category="DEDUP")
    log:    "LOGS/{file}/fastqc_dedup_{cond}.log"
    conda: "../envs/qc.yaml"
    threads: 20
    params: dir= expand("QC/{source}",source=SOURCE)
    shell: "for i in {input}; do OUT=$(dirname {output});fastqc --quiet -o $OUT -t {threads} --noextract -f bam {input} 2> {log};done"

rule fastqc_uniqdedup:
    input:  "UNIQUE_MAPPED/{file}_mapped_{cond}_sorted_unique_dedup.bam"
    output: report("QC/{file}_mapped_{cond}_sorted_unique_dedup_fastqc.zip", category="QC")
    log:    "LOGS/{file}/fastqc_uniqdedup_{cond}.log"
    conda: "../envs/qc.yaml"
    threads: 20
    params: dir= expand("QC/{source}", source=SOURCE)
    shell: "for i in {input}; do OUT=$(dirname {output});fastqc --quiet -o $OUT -t {threads} --noextract -f bam {input} 2> {log};done"
...
rule fastqc_phaseddedup:
    input:  "PHASED_MAPPED/{file}_mapped_{cond}_sorted_phased_dedup.bam"
    output: report("QC/{file}_mapped_{cond}_sorted_phased_dedup_fastqc.zip", category="QC")
    log:    "LOGS/{file}/fastqc_phaseddedup_{cond}.log"
    conda: "../envs/qc.yaml"
    threads: 20
    params: dir= expand("QC/{source}", source=SOURCE)
    shell: "for i in {input}; do OUT=$(dirname {output});fastqc --quiet -o $OUT -t {threads} --noextract -f bam {input} 2> {log};done"
...
rule multiqc:
    input:  expand("QC/{file}_fastqc.zip",file=SAMPLES),
            ...
            expand("QC/{file}_mapped_cluster_sorted_dedup_fastqc.zip",file=SAMPLES),
            ...
            expand("QC/{file}_mapped_artificial_sorted_dedup_fastqc.zip",file=SAMPLES)
...
rule themall:
    input:  "QC/Multi/multiqc_report.html", "PHASED_MAPPED/{file}_mapped_cluster_sorted_phased_dedup.bam" if config["QC"] == "ON" else "PHASED_MAPPED/{file}_mapped_cluster_sorted_phased_dedup.bam"
