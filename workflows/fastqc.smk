ruleorder: qc_raw > qc_trimmed

if paired == 'paired':
    log.info('Running paired mode QC')
    rule qc_raw:
        input:  r1 = expand("FASTQ/{rawfile}_{read}.fastq.gz", rawfile=SAMPLES, read=['R1','R2'])
        output: o1 = report(expand("QC/{rawfile}_{read}_fastqc.zip", rawfile=SAMPLES, read=['R1','R2']), category="QC")
        wildcard_constraints:
            rawfile="!trimmed"
        log:    expand("LOGS/{rawfile}/fastqc_{read}_raw.log", rawfile=SAMPLES, read=['R1','R2'])
        conda:  "snakes/envs/qc.yaml"
        threads: MAXTHREAD
        params: dir=lambda w: expand("QC/{source}", source=source_from_sample(SAMPLES[0],config)),
                qpara = lambda w: ' '.join("{!s} {!s}".format(key,val) for (key,val) in tool_params(SAMPLES[0], None ,config, 'QC')['OPTIONS'][0].items())
    ...
    rule qc_raw:
        ...
        output: o1 = report(expand("QC/{rawfile}_fastqc.zip", rawfile=SAMPLES), category="QC")
        wildcard_constraints:
            rawfile="!trimmed"
        log:    expand("LOGS/{rawfile}/fastqc_raw.log", rawfile=SAMPLES)
        conda:  "snakes/envs/qc.yaml"
        threads: MAXTHREAD
        params: dir=lambda w: expand("QC/{source}", source=source_from_sample(SAMPLES[0],config)),
