    ...
    if analysis == 'DEU':
        ...
        rule prepare_count_table:
            ...
            params:  decond = lambda wildcards, input: str.join(',',[','.join(tool_params(str.join(os.sep, x.split(os.sep)[2:]).replace('_mapped_sorted_unique.counts',''), None, config, 'DEU')['CONDITION']) for x in input.cnd]),
                     ...
                     detypes = lambda wildcards, input: str.join(',',[','.join(tool_params(str.join(os.sep, x.split(os.sep)[2:]).replace('_mapped_sorted_unique.counts',''), None, config, 'DEU')['TYPES']) for x in input.cnd]),
                     samples = lambda wildcards, input: str.join(',',input.cnd),
                     bins = BINS
            shell: "{params.bins}/Analysis/DEU/build_DEXSeq_table.py -l {params.samples} -r {params.dereps} -c {params.decond} -t {paramas.types} --table {output.tbl} --anno {output.anno} 2> {log}"
