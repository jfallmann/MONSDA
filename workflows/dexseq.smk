...
rule filter_significant_dexseq:
    ...
    conda:  "nextsnakes/envs/"+DEUENV+".yaml"
    threads: 1
    shell: "for i in {outdir}DEXSeq_*.tsv.gz;do fn=\"${{i##*/}}\"; if [[ -s \"$i\" ]];then zcat $i| tail -n+2 |grep -v -w 'NA'|perl -F$'\t' -wlane 'next if (!$F[7] || !$F[10]);if ($F[7] < 0.05 && ($F[10] <= -1.5 ||$F[10] >= 1.5) ){{print}}' |gzip > {outdir}Sig_$fn && zcat $i| tail -n+2 |grep -v -w 'NA'|perl -F$'\t' -wlane 'next if (!$F[7] || !$F[10]);if ($F[7] < 0.05 && ($F[10] >= 1.5) ){{print}}' |gzip > {outdir}SigUP_$fn && zcat $i| tail -n+2 |grep -v -w 'NA'|perl -F$'\t' -wlane 'next if (!$F[7] || !$F[10]);if ($F[7] < 0.05 && ($F[10] <= -1.5) ){{print}}' |gzip > {outdir}SigDOWN_$fn;else touch {outdir}Sig_$fn {outdir}SigUP_$fn {outdir}SigDOWN_$fn; fi;done 2> {log}"
