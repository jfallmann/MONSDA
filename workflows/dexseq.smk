...
rule featurecount_dexseq_unique:
    ...
    shell:  "{params.count} -T {threads} {params.cpara} {params.paired} {params.stranded} -a <(zcat {params.anno}) -o {output.tmp} {input.reads} 2> {log} && head -n2 {output.tmp} > {output.cts} && tail -n+3 {output.tmp}|sort -k1,1 -k2,2n -k3,3n -u >> {output.cts}"
