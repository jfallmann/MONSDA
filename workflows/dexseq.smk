for analysis in ['DE', 'DEU', 'DAS']:
    ...
    if analysis == 'DEU':
        rule featurecount_unique:
            ...
            shell:  "if [ ! -f \"{params.tmpgtf}\" ];then {params.bins}/Analysis/"+analysis+"/prepare_dexseq_annotation2.py {params.anno} {params.tmpgtf} ;fi && ln -s {params.tmpgtf} {output[1]} && {params.count} -T {threads} {params.cpara} {params.paired} {params.stranded} -a {output[1]} -o {output[0]} {input[0]} 2> {log}"
