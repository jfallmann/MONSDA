...
rule featurecount_dexseq_unique:
    ...
    shell:  "{params.countb} -T {threads} {params.cpara} {params.paired} {params.stranded} -a <(zcat {input.countgtf}) -o {output.tmp} {input.reads} 2> {log} && head -n2 {output.tmp} > {output.cts} && export LC_ALL=C; tail -n+3 {output.tmp}|sort --parallel={threads} -S 25% -T TMP -k1,1 -k2,2n -k3,3n -u >> {output.cts} && mv {output.tmp}.summary {output.cts}.summary"
...
rule filter_significant_dexseq:
    ...
    conda:  "nextsnakes/envs/"+DEUENV+".yaml"
    threads: 1
    shell: "cd {outdir} && for i in DEXSeq_*.tsv.gz;do if [ -s \"$i\" ];then zcat $i| tail -n+2 |grep -v -w 'NA'|perl -F$'\t' -wlane 'next if (!$F[7] || !$F[10]);if ($F[7] < 0.05 && ($F[10] <= -1.5 ||$F[10] >= 1.5) ){{print}}' |gzip > Sig_$i && zcat $i| tail -n+2 |grep -v -w 'NA'|perl -F$'\t' -wlane 'next if (!$F[7] || !$F[10]);if ($F[7] < 0.05 && ($F[10] >= 1.5) ){{print}}' |gzip > SigUP_$i && zcat $i| tail -n+2 |grep -v -w 'NA'|perl -F$'\t' -wlane 'next if (!$F[7] || !$F[10]);if ($F[7] < 0.05 && ($F[10] <= -1.5) ){{print}}' |gzip > SigDOWN_$i;else touch Sig_$i SigUP_$i SigDOWN_$i; fi;done 2> {log}"
