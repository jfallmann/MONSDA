...
IDXPARAMS = get_always('rustar_params_INDEX') ?: ''
MAPPARAMS = get_always('rustar_params_MAP') ?: ''
...
process rustar_idx{
    ...
    saveAs: {filename ->
        if (filename.indexOf("Log.out") > 0)             "LOGS/${COMBO}/${CONDITION}/MAPPING/star/rustar_index.log"
    ...
    """
    zcat $gen > tmp.fa && zcat $an > tmp_anno && mkdir -p $MAPUIDXNAME && $MAPBIN $IDXPARAMS --runThreadN ${task.cpus} --runMode genomeGenerate --outTmpDir RUSTARTMP --genomeDir $MAPUIDXNAME --genomeFastaFiles tmp.fa && touch $MAPUIDXNAME && ln -s $MAPUIDXNAME rustar.idx && rm -f tmp.fa tmp_anno && ln -fs $MAPUIDXNAME/* . && rm -rf RUSTARTMP
    """
}
...
process rustar_mapping{
    ...
    saveAs: {filename ->
        ...
        else if (filename.indexOf(".log") >0)        "LOGS/${COMBO}/${CONDITION}/MAPPING/star/rustar_"+"${file(filename).getName()}"
        else if (filename.indexOf(".out") >0)        "LOGS/${COMBO}/${CONDITION}/MAPPING/star/rustar_"+"${file(filename).getName()}"
