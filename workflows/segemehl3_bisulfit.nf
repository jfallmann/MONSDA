...
process segemehl3_mapping{
    ...
    if (PAIRED == 'paired'){
        ...
        lf = "segemehl_"+fn+".log"
        """
        $MAPBIN $MAPPARAMS --threads $THREADS -i $idx -j $idx2 -d $gen -q $r1 -p $r2 -o $pf -u $uf &> $lf && touch $uf && gzip *.sam
        """
    }else{
        ...
        lf = "segemehl_"+fn+".log"
        """
        $MAPBIN $MAPPARAMS --threads $THREADS -i $idx -j $idx2 -d $gen -q $read -o $pf -u $uf  &> $lf && touch $uf && gzip *.sam
        """
    }
}
