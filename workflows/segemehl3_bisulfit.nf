...
process segemehl3_mapping{
    ...
    if (PAIRED == 'paired'){
        ...
        lf = "segemehl_"+fn+".log"
        """
        $MAPBIN $MAPPARAMS --threads $THREADS -i $idx -j $idx2 -d $gen -q $r1 -p $r2 2> lf | tee >(samtools view -h -F 4 |gzip > $pf) >(samtools view -h -f 4 |samtools fastq -n - | pigz > $uf) 1>/dev/null 2>> $lf && touch $uf
        """
    }else{
        ...
        lf = "segemehl_"+fn+".log"
        """
        $MAPBIN $MAPPARAMS --threads $THREADS -i $idx -j $idx2 -d $gen -q $read  2> lf| tee >(samtools view -h -F 4 |gzip > $pf) >(samtools view -h -f 4 |samtools fastq -n - | pigz > $uf) 1>/dev/null 2>> $lf && touch $uf
        """
    }
}
