...
rule dedupbam:
    ...
    params: jpara = lambda wildcards: tool_params(wildcards.file, None, config, "DEDUP", DEDUPENV)['OPTIONS'].get('JAVA', ""),
            ...
            dedup = DEDUPBIN
    shell: "mkdir -p {output.td} && {params.dedup} {params.jpara} MarkDuplicates --REMOVE_DUPLICATES=TRUE --ASSUME_SORT_ORDER=coordinates --TMP_DIR={output.td} INPUT={input.bam} OUTPUT={output.bam} 2>> {log}"
