rule generate_index:
    ...
    shell: "zcat {input.fa} > {output.tmp} && zcat {params.anno} > {output.tmpa} && rm -rf STARTMP && {params.mapp} {params.ipara} --runThreadN {threads} --runMode genomeGenerate --outTmpDir STARTMP --genomeDir {params.genpath} --genomeFastaFiles {output.tmp} --sjdbGTFfile {output.tmpa} --sjdbGTFtagExonParentTranscript Parent  2> {log} && cat LOG.out >> {log} && rm -f LOG.out"
