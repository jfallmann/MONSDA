...
process PreprocessPeaks{
    ...
    script: 
    of = file(bed).getSimpleName().replaceAll(/\Q_mapped\E/,"_prepeak").replaceAll(/\Q_ext.\E/,".")+".bed.gz"
...
workflow PEAKS{ 
    ...
    BamToBed(mapsamples_ch, bamindex_ch)
    ExtendBed(BamToBed.out.bed.combine(UnzipGenome.out.chromsize))
    BedToBedg(BamToBed.out.bed.concat(ExtendBed.out.bedext).combine(UnzipGenome.out.index.combine(UnzipGenome.out.chromsize)))    
    ...
    BedToBedgNoSoft(ExtendBed.out.bedext.combine(UnzipGenome.out.index.combine(UnzipGenome.out.chromsize)))
    PreprocessPeaks(BedToBedgNoSoft.out.bedg)
