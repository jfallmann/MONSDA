...
rule call_base:
    ...
    shell: " echo \"{params.f5file}\" > {params.fqdir}/f5list && {params.caller} {params.cpara} -c {params.cmodel} --compress_fastq -i {params.f5dir} --input_file_list {params.fqdir}/f5list -s {params.f5dir} 2> {log} && cat {params.f5dir}/pass/fastq_runid_*.fastq.gz > {output.fq} && rm -f {params.f5dir}/pass/fastq_runid_*.fastq.gz && cat {params.f5dir}/*.log >> {log} && rm -f {params.f5dir}/*.log && mv -f {params.f5dir}/sequencing_summary.txt {output.summary} &&  mv -f {params.f5dir}/sequencing_telemetry.js {output.telemetry} && rm -f {params.f5dir}/f5list"