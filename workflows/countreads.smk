...
if not rundedup:
    rule themall:
        input:  expand("COUNTS/Featurecounts_{feat}s/{combo}{file}_mapped_sorted_unique.counts", file=samplecond(SAMPLES,config), feat=config['COUNTING']['FEATURES'].keys()),
                expand("COUNTS/Featurecounts_{feat}s/{combo}{file}_mapped_sorted.counts", file=samplecond(SAMPLES,config), feat=config['COUNTING']['FEATURES'].keys()),
                expand("COUNTS/{combo}{file}.summary", file=samplecond(SAMPLES,config))
else:
    rule themall:
        input:  expand("COUNTS/Featurecounts_{feat}s/{combo}{file}_mapped_sorted.counts", file=samplecond(SAMPLES,config), feat=config['COUNTING']['FEATURES'].keys()),
                expand("COUNTS/Featurecounts_{feat}s/{combo}{file}_mapped_sorted_dedup.counts", file=samplecond(SAMPLES,config), feat=config['COUNTING']['FEATURES'].keys()),
                expand("COUNTS/Featurecounts_{feat}s/{combo}{file}_mapped_sorted_unique.counts", file=samplecond(SAMPLES,config), feat=config['COUNTING']['FEATURES'].keys()),
                expand("COUNTS/Featurecounts_{feat}s/{combo}{file}_mapped_sorted_unique_dedup.counts", file=samplecond(SAMPLES,config), feat=config['COUNTING']['FEATURES'].keys()),
                expand("COUNTS/{combo}{file}.summary", file=samplecond(SAMPLES,config))
...
if paired == 'paired':
    ...
    rule count_fastq:
        input:  r1 = lambda wildcards: "FASTQ/{rawfile}.fastq.gz".format(rawfile=[x for x in SAMPLES if x.split(os.sep)[-1] in wildcards.file][0]),
                r2 = "TRIMMED_FASTQ/{combo}{file}_trimmed.fastq.gz"
...
rule count_mappers:
    input:  m = "MAPPED/{combo}{file}_mapped_sorted.bam"
...
rule count_unique_mappers:
    input:  u = "MAPPED/{combo}{file}_mapped_sorted_unique.bam"
...
rule count_dedup_mappers:
    input:  m = "MAPPED/{combo}{file}_mapped_sorted_dedup.bam"
...
rule count_unique_dedup_mappers:
    input:  u = "MAPPED/{combo}{file}_mapped_sorted_unique_dedup.bam"
...
rule featurecount:
    input:  s = "MAPPED/{combo}{file}_mapped_sorted.bam",
...
rule featurecount_dedup:
    input:  s = "MAPPED/{combo}{file}_mapped_sorted_dedup.bam",
