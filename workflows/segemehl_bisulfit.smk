...
if paired == 'paired':
    rule mapping:
        input:  r1 = "TRIMMED_FASTQ/{combo}/{file}_R1_trimmed.fastq.gz",
                ...
                ref = REFERENCE
        output: mapped = report("MAPPED/{combo}/{file}_mapped.sam.gz", category="MAPPING"),
        ...
        shell: "{params.mapp} {params.mpara} -d {input.ref} -i {input.uidx1} -j {input.uidx2} -q {input.r1} -p {input.r2} --threads {threads} -o {params.muz} -u {params.uuz} &> {log} && gzip {params.muz} && gzip {params.uuz}"
    ...
    rule mapping:
        ...
        params: mpara = lambda wildcards: tool_params(wildcards.file, None, config, 'MAPPING', MAPPERENV)['OPTIONS'].get('MAP', ""),
                ...
                muz = lambda wildcards, output: output.mapped.replace('.gz', ''),
                uuz = lambda wildcards, output: output.unmapped.replace('.gz', '')
        shell: "{params.mapp} {params.mpara} -d {input.ref} -i {input.uidx1} -j {input.uidx2} -q {input.query} --threads {threads} -o {params.muz} -u {params.uuz} &> {log} && gzip {params.muz} && gzip {params.uuz}"
