...
rule filter_significant_deseq2:
    ...
    conda:  "nextsnakes/envs/"+DEENV+".yaml"
    threads: 1
    shell: "cd {outdir} && for i in DESeq2_*.tsv.gz;do if [ -s \"$i\" ];then zcat $i| tail -n+2 |grep -v -w 'NA'|perl -F$'\t' -wlane 'next if (!$F[2] || !$F[6]);if ($F[6] < 0.05 && ($F[2] <= -1.5 ||$F[2] >= 1.5) ){{print}}' |gzip > Sig_$i && zcat $i| tail -n+2 |grep -v -w 'NA'|perl -F$'\t' -wlane 'next if (!$F[2] || !$F[6]);if ($F[6] < 0.05 && ($F[2] >= 1.5) ){{print}}' |gzip > SigUP_$i && zcat $i| tail -n+2 |grep -v -w 'NA'|perl -F$'\t' -wlane 'next if (!$F[2] || !$F[6]);if ($F[6] < 0.05 && ($F[2] <= -1.5) ){{print}}' |gzip > SigDOWN_$i; else touch Sig_$i SigUP_$i SigDOWN_$i; fi;done 2> {log}"
