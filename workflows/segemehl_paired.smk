rule generate_index_paired:
    ...
    params: mapp = MAPPERBIN,
            ...
            genpath = lambda wildcards: "{refe}/{genname}".format(refe=REFERENCE,genname=os.path.dirname(wildcards.dir))
...
rule mapping_paired:
    ...
    params: mpara = lambda wildcards: ' '.join("{!s} {!s}".format(key,val) for (key,val) in tool_params(wildcards.file, None , config, "MAPPING")[1].items()),
            mapp=MAPPERBIN
    shell: "{params.mapp} {params.mpara} -d {input.ref} -i {input.index} -q {input.r1} -p {input.r2} --threads {threads} -o {output[0]} -u {output[1]} 2> {log}"
