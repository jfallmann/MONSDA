...
rule themall:
    input:  all = expand("{outdir}EDGER_DE_All_Conditions_MDS.png", outdir=outdir),
            ...
            sigdift = expand("{outdir}Sig_EDGER_DE_{comparison}_genes_{sort}.tsv.gz", outdir=outdir, comparison=compstr, sort=["logFC-sorted","pValue-sorted"]),
...
rule filter_significant_edgerDE:
    ...
    conda:  "nextsnakes/envs/"+DEENV+".yaml"
    threads: 1
    shell: "cd {outdir} && for i in EDGER_*.tsv.gz;do if [ -s \"$i\" ];then for i in EDGER_DE*pValue-sorted.tsv.gz;do zcat $i| tail -n+2 |grep -v -w 'NA'|perl -F$'\t' -wlane 'next if (!$F[2] || !$F[6]);if ($F[6] < 0.05 && ($F[2] <= -1.5 ||$F[2] >= 1.5) ){{print}}' |gzip > $i\_Sig.tsv.gz && zcat $i| tail -n+2 |grep -v -w 'NA'|perl -F$'\t' -wlane 'next if (!$F[2] || !$F[6]);if ($F[6] < 0.05 && ($F[2] >= 1.5) ){{print}}' |gzip > $i\_SigUP.tsv.gz && zcat $i| tail -n+2 |grep -v -w 'NA'|perl -F$'\t' -wlane 'next if (!$F[2] || !$F[6]);if ($F[6] < 0.05 && ($F[2] <= -1.5) ){{print}}' |gzip > $i\_SigDOWN.tsv.gz;else touch Sig_$i SigUP_$i SigDOWN_$i; fi;done 2> {log}"
