DEBIN, DEENV = env_bin_from_config(config, 'DE')
...
rule themall:
    input:  session = expand("DE/{combo}/DE_EDGER_{scombo}_SESSION.gz", combo=combo, scombo=scombo),
            ...
            sort    = expand("DE/{combo}/Tables/DE_EDGER_{scombo}_{comparison}_table_results{sort}.tsv.gz", combo=combo, comparison=compstr, scombo=scombo, sort=["PValueSorted"]) if not usespike else expand("DE/{combo}/Tables/DE_EDGER_{scombo}_{comparison}_table_results{sort}_norm.tsv.gz", combo=combo, comparison=compstr, scombo=scombo, sort=["PValueSorted"]),
...
rule featurecount_unique:
    ...
    log:    "LOGS/{combo}/{file}/DE/edger/featurecounts_edger_unique.log"
    conda:  ""+COUNTENV+".yaml"
...
rule prepare_count_table:
    ...
    log:     "LOGS/{combo}/DE/edger/{scombo}_prepare_count_table.log"
    conda:   ""+DEENV+".yaml"
...
rule run_edger:
    ...
    log:    expand("LOGS/{combo}/DE/edger/run_edger.log", combo=combo)
    conda:  ""+DEENV+".yaml"
    container: "oras://jfallmann/monsda:"+DEENV+""
...
rule filter_significant:
    ...
    log:    expand("LOGS/{combo}/DE/edger/filter_edgerDE.log", combo=combo)
    conda:  ""+DEENV+".yaml"
    container: "oras://jfallmann/monsda:"+DEENV+""
    threads: 1
...
rule create_summary_snippet:
    ...
    log:    expand("LOGS/{combo}/DE/edger/create_summary_snippet.log", combo=combo)
    conda:  ""+DEENV+".yaml"
    container: "oras://jfallmann/monsda:"+DEENV+""
    threads: int(MAXTHREAD-1) if int(MAXTHREAD-1) >= 1 else 1
    params: bins = BINS,
