...
rule filter_significant:
    ...
    shell:  "set +o pipefail; a={input.tbl}; if [[ -s \"$a\" ]];then zcat $a| head -n1 |gzip > {output.sig}; cp {output.sig} {output.sig_d}; cp {output.sig} {output.sig_u}; zcat $a| tail -n+2 |grep -v -w 'NA'|perl -F\'\\t\' -wlane 'next if (!$F[6] || !$F[3]);if ($F[3] < {params.pv_cut} && ($F[6] <= -{params.lfc_cut} ||$F[6] >= {params.lfc_cut}) ){{print}}' |gzip >> {output.sig} && zcat $i| tail -n+2 |grep -v -w 'NA'|perl -F\'\\t\' -wlane 'next if (!$F[6] || !$F[3]);if ($F[3] < {params.pv_cut} && ($F[6] >= {params.lfc_cut}) ){{print}}' |gzip >> {output.sig_u} && zcat $i| tail -n+2 |grep -v -w 'NA'|perl -F\'\\t\' -wlane 'next if (!$F[6] || !$F[3]);if ($F[3] < {params.pv_cut} && ($F[6] <= -{params.lfc_cut}) ){{print}}' |gzip >> {output.sig_d}; else touch {output.sig} {output.sig_d} {output.sig_u} ; fi 2>> {log}"
