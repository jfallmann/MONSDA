if paired == 'paired':
    rule simulate_trim:
        input:  r1 = expand("FASTQ/{rawfile}_R1.fastq.gz", rawfile=SAMPLES) if not rundedup else "DEDUP_FASTQ/{file}_R1_dedup.fastq.gz",
                r2 = expand("FASTQ/{rawfile}_R2.fastq.gz", rawfile=SAMPLES) if not rundedup else "DEDUP_FASTQ/{file}_R2_dedup.fastq.gz"
    ...
    rule simulate_trim:
        input:  r1 = expand("FASTQ/{rawfile}.fastq.gz", rawfile=SAMPLES) if not rundedup else "DEDUP_FASTQ/{file}_dedup.fastq.gz"
