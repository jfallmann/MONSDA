if paired == 'paired':
    rule simulate_trim:
        input:  r1 = expand("FASTQ/{rawfile}_R1.fastq.gz", rawfile=SAMPLES) if not config.get('DEDUP') else "DEDUP_FASTQ/{file}_R1.fastq.gz",
                r2 = expand("FASTQ/{rawfile}_R2.fastq.gz", rawfile=SAMPLES) if not config.get('DEDUP') else "DEDUP_FASTQ/{file}_R2.fastq.gz"
    ...
    rule simulate_trim:
        input:  r1 = expand("FASTQ/{rawfile}.fastq.gz", rawfile=SAMPLES) if not config.get('DEDUP') else "DEDUP_FASTQ/{file}_R1.fastq.gz"
