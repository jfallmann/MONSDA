...
if paired == 'paired':
    ...
    rule multiqc:
        ...
        output: html = report("{moutdir}TRIMMED_RAW/{condition}/multiqc_report.html", category="QC"),
                tmp = temp("{moutdir}TRIMMED_RAW/{condition}/tmp"),
                lst = "{moutdir}TRIMMED_RAW/{condition}/qclist.txt"
        log:    "LOGS/{moutdir}{condition}_multiqc_trimmed_raw.log"
        conda:  "nextsnakes/envs/"+QCENV+".yaml"
        threads: 1
