...
rule filter_significant_drimseq:
    ...
    shell:  "set +o pipefail; array1=({input.tbl}); array2=({output.sig_g}); array3=({output.sig_dg}); array4=({output.sig_ug}); array5=({output.sig_t}); array6=({output.sig_dt}); array4=({output.sig_ut}); for i in ${{!array1[@]}}; do a=${{array1[$i]}}; fn=\"${{a##*/}}\"; if [[ -s \"$a\" ]];then zcat $a| head -n1 |gzip > ${{array2[$i]}}; cp ${{array2[$i]}} ${{array3[$i]}}; cp ${{array2[$i]}} ${{array4[$i]}}; zcat $a| tail -n+2 |grep -v -w 'NA'|perl -F\'\\t\' -wlane 'next if (!$F[2] || !$F[1]);if ($F[1] < {params.pv_cut} && ($F[2] <= -{params.lfc_cut} ||$F[2] >= {params.lfc_cut}) ){{print}}' |gzip >> ${{array2[$i]}} && zcat $i| tail -n+2 |grep -v -w 'NA'|perl -F\'\\t\' -wlane 'next if (!$F[2] || !$F[1]);if ($F[1] < {params.pv_cut} && ($F[2] >= {params.lfc_cut}) ){{print}}' |gzip >> ${{array3[$i]}} && zcat $i| tail -n+2 |grep -v -w 'NA'|perl -F\'\\t\' -wlane 'next if (!$F[2] || !$F[1]);if ($F[1] < {params.pv_cut} && ($F[2] <= -{params.lfc_cut}) ){{print}}' |gzip >> ${{array4[$i]}}; else touch ${{array2[$i]}} ${{array3[$i]}} ${{array4[$i]}}; fi;done 2> {log}"
