...
rule call_base:
    ...
    shell: "{params.caller} download --directory {params.p5dir} --model {params.cmodel} &> {log} && {params.caller} basecaller {params.cpara} {params.p5dir}/{params.cmodel} {params.p5dir}/ 2>> {log} 1> {output.bam} && samtools view -h {output.bam}|samtools fastq -n - | pigz > {output.fq} && rm -f {params.fqdir}/fastq_runid_*.fastq.gz && cat {params.fqdir}/*.log >> {log} && rm -f {params.fqdir}/*.log && mv -f {params.fqdir}/sequencing_summary.txt {output.summary} &&  mv -f {params.fqdir}/sequencing_telemetry.js {output.telemetry} && rm -f {params.fqdir}/f5list"