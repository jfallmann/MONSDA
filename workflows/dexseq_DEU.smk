...
rule themall:
    input:  session = expand("{outdir}/DEU_DEXSEQ_{combi}_SESSION.gz", outdir=outdir, combi=combi),
...
rule run_dexseq:
    ...
    params: bins   = str.join(os.sep,[BINS, DEUBIN]),
            ...
            combi = combi,
            ref = ANNOTATION
    shell: "Rscript --no-environ --no-restore --no-save {params.bins} {input.anno} {input.cnt} {params.ref} {input.flat} {params.outdir} {params.combi} {params.compare} {threads} 2> {log}"

rule filter_significant_dexseq:
    input:  tbl = rules.run_dexseq.output.tbl
    output: sigtbl  = expand("{outdir}/Sig_DEU_DEXSEQ_{comparison}_results.tsv.gz", outdir=outdir, comparison=compstr)
