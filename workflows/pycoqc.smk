...
rule pycoqc_raw:
    ...
    output: report("QC/{file}_pycoqc.zip", category="QC")
    log:    "LOGS/{file}/pycoqc_raw.log"
    conda:  "../envs/qc.yaml"
    threads: 20
...
rule pycoqc_trimmed:
    input:  "TRIMMED_FASTQ/{file}_trimmed.fastq.gz",
            "QC/{file}_pycoqc.zip"
    output: report("QC/{file}_trimmed_pycoqc.zip", category="QC")
    log:    "LOGS/{file}/pycoqc_trimmed.log"
    conda:  "../envs/qc.yaml"
    threads: 20
    params: dir=lambda w: expand("QC/{source}",source=source_from_sample(w.file))
