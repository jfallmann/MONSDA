...
for(contrast in comparisons[[1]]){
    ...
    tryCatch({
        ...
        fit <- fit[1:250,] # reduce data for testing - REMOVE!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        tops <- topTags(is.de, n=nrow(qlf$table), sort.by="logFC")
        ...
        write.table(as.data.frame(qlf), gzfile(paste("SUMTABLE","_DE","_EDGER_",contrast_name,".tsv.gz", sep="")), sep="\t", quote=F, row.names=FALSE)
