...
#		readnum = 0	
		...
		if 'SQ' in header:
			...
			for chrom in chroms:
				pool.apply_async(collect, args=(bam, fasta, out, chrom))#, callback = log_result)
		else:
#			res = pool.apply_async(test,args=(27,))
			res = pool.apply_async(collect, args=(bam, fasta, out,))#, callback = log_result)
	...
	if chrom is not 'None':
		for read in samfile.fetch(chrom):
			if not read.is_unmapped:
				...
				if (len(statistics['reads']) > 0) and (chrom not in statistics['reads']):# or (alignmentstart not in statistics['reads'][chrom])):
...
def write_header(bam):
	head = str.join('\t',['Chromosome','Position','Ref','A','C','G','T','N','RelRefFreq'])+'\n'
	o = gzip.open(bam,'ab')
		...
		for pos in statistics['reads'][chrom]:
			if start is not None:
				if int(pos) > int(start):
					continue
			outstr = str.join('\t',[str(chrom),str(pos),statistics['ref'][chrom][pos]])+'\t'
			sum = 0
			...
			if statistics['ref'][chrom][pos] in statistics['reads'][chrom][pos]:
				coverage += str(statistics['reads'][chrom][pos][statistics['ref'][chrom][pos]]/sum)
			else:
				coverage += str(0)
			outstr += coverage+'\n'			
