...
def parseargs():
    ...
    parser.add_argument(
        "--anno", dest='anno', required=True, type=str, default='counts.anno', help="Name of anno to write to"
    )
...
def prepare_table(
    conditions, replicates, types, batches, paired, table, anno, sample_name=None, order=None, cutoff=None
):
    try:
        # slist,
                ...
                for line in myInput:
                    ...
                    if columns[0] != "name" and columns[1] != "count":
                        ...
                        if gene != oldgene:
                            ...
                            junctionnr += 1
                        chr = (
                            str(columns[1]).replace(':', "|") + '-' + str(columns[2]) + '-' + str(columns[3])
                        )
