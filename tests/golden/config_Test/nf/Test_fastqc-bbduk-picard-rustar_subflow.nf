...
params.gIDXPARAMS = get_always('rustar_params_INDEX') ?: ''
params.gMAPPARAMS = get_always('rustar_params_MAP') ?: ''
...
process rustar_idx{
    ...
    saveAs: {filename ->
        if (filename.indexOf("Log.out") > 0)             "LOGS/${params.gCOMBO}/${params.gCONDITION}/MAPPING/star/rustar_index.log"
    ...
    an  = anno.getName()
    """
    zcat $gen > tmp.fa && zcat $an > tmp_anno && mkdir -p ${params.gMAPUIDXNAME} && ${params.gMAPBIN} ${params.gIDXPARAMS} --runThreadN ${task.cpus} --runMode genomeGenerate --outTmpDir RUSTARTMP --genomeDir ${params.gMAPUIDXNAME} --genomeFastaFiles tmp.fa && touch ${params.gMAPUIDXNAME} && ln -s ${params.gMAPUIDXNAME} rustar.idx && rm -f tmp.fa tmp_anno && ln -fs ${params.gMAPUIDXNAME}/* . && rm -rf RUSTARTMP
    """
}
process rustar_mapping{
    conda "<REPO>/envs/${params.gMAPENV}"+".yaml"
    container "oras://ghcr.io/jfallmann/monsda:"+"${params.gMAPENV}"+"-VERSION"
    cpus params.gTHREADS
	cache 'lenient'
    ...
    saveAs: {filename ->
        ...
        else if (filename.indexOf(".log") >0)        "LOGS/${params.gCOMBO}/${params.gCONDITION}/MAPPING/star/rustar_"+"${file(filename).getName()}"
        else if (filename.indexOf(".out") >0)        "LOGS/${params.gCOMBO}/${params.gCONDITION}/MAPPING/star/rustar_"+"${file(filename).getName()}"
...
workflow MAPPING{
    ...
    if (checkidx.exists()){
        ...
        rustar_mapping(idxfile.combine(collection))
    }
    else{
        genomefile = Channel.fromPath(params.gMAPREF)
        annofile = Channel.fromPath(params.gMAPANNO)
        rustar_idx(genomefile, annofile)
        rustar_mapping(rustar_idx.out.idx.combine(collection))
    }
    emit:
    mapped  = rustar_mapping.out.maps
    logs = rustar_mapping.out.logs
}
