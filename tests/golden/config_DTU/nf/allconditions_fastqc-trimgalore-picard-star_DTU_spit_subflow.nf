...
process salmon_idx{
    ...
    publishDir "${workflow.workDir}/../" , mode: 'copyNoFollow',
    saveAs: {filename ->
        if (filename.indexOf(".log") >0)    "LOGS/${params.gCOMBO}/${params.gCONDITION}/DTU/spit/index.log"
...
process prepare_dtu_annotation{
    ...
    saveAs: {filename ->
        ...
        else if (filename.indexOf(".log") > 0)        "LOGS/${params.gSCOMBO}/DTU/spit/featurecount_spit_annotation.log"
    }
    output:
    path "*.gz", emit: anno
    path "*.log", emit: log
...
process run_spit{
    ...
    saveAs: {filename ->
        ...
        else if (filename.indexOf("log") > 0)        "LOGS/${params.gSCOMBO}/DTU/spit/run_spit.log"
    }
    input:
    path counts
    path anno
