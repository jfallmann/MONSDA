...
params.gIDXPARAMS = get_always('drimseq_DTU_params_INDEX') ?: ''
params.gCOUNTPARAMS = get_always('drimseq_DTU_params_COUNT') ?: ''
params.gDTUPARAMS = get_always('drimseq_DTU_params_DTU') ?: ''
params.gRUNTERMINUS = get_always('drimseq_DTU_params_TERMINUS') ?: ''
...
process salmon_idx{
    ...
    publishDir "${workflow.workDir}/../" , mode: 'copyNoFollow',
    saveAs: {filename ->
        if (filename.indexOf(".log") >0)    "LOGS/${params.gCOMBO}/${params.gCONDITION}/DTU/drimseq/index.log"
        else if (filename == "drimseq.idx")           "${params.gDTUIDX}"
    ...
    """
    ${params.gCOUNTBIN} index ${params.gIDXPARAMS} $decoy -p ${task.cpus} -t $gen -i ${params.gDTUUIDXNAME} &> index.log && ln -fs ${params.gDTUUIDXNAME} drimseq.idx
    """
}
process prepare_dtu_annotation{
    ...
    saveAs: {filename ->
        ...
        else if (filename.indexOf(".log") > 0)        "LOGS/${params.gSCOMBO}/DTU/drimseq/featurecount_drimseq_annotation.log"
    }
    output:
    path "*.gz", emit: anno
    path "*.log", emit: log
...
process run_drimseq{
    ...
    saveAs: {filename ->
        ...
        else if (filename.indexOf(".html") > 0)      "DTU/${params.gSCOMBO}/drimseqReport_${params.gCOMBO}_${params.gDTUCOMP}/${file(filename).getName()}"
        else if (filename.indexOf("SESSION") > 0)      "DTU/${params.gSCOMBO}/${file(filename).getName()}"                     
        else if (filename.indexOf("log") > 0)        "LOGS/${params.gSCOMBO}/DTU/drimseq/run_drimseq.log"
    }
    input:
    path counts
    path anno
    ...
    Rscript --no-environ --no-restore --no-save $bin $anno $ref . ${params.gDTUCOMP} ${params.gPCOMBO} ${task.cpus} $dparams &> log ; ln -f Tables/* . && touch Figures/dummy() && ln -f Figures/* .
    """
}
