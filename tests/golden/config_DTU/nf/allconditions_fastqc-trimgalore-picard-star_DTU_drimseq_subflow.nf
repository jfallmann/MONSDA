...
process salmon_idx{
    ...
    publishDir "${workflow.workDir}/../" , mode: 'copyNoFollow',
    saveAs: {filename ->
        if (filename.indexOf(".log") >0)    "LOGS/${params.gCOMBO}/${params.gCONDITION}/DTU/drimseq_index.log"
...
process prepare_dtu_annotation{
    ...
    saveAs: {filename ->
        ...
        else if (filename.indexOf(".log") > 0)        "LOGS/DTU/${params.gSCOMBO}/featurecount_drimseq_annotation.log"
    }
    output:
    path "*.gz", emit: anno
    path "*.log", emit: log
...
process run_drimseq{
    ...
    saveAs: {filename ->
        ...
        else if (filename.indexOf("log") > 0)        "LOGS/DTU/${params.gSCOMBO}/run_drimseq.log"
    }
    input:
    path counts
    path anno
